...
### Insertion d'un document (`a`)

Depuis le Data Grid MongoDB (mode non read-only), appuyez sur `a` pour créer un nouveau document :

- Les champs sont inférés depuis la **première ligne existante** de la collection (noms + types réels)
- Les champs `[obj]` sont initialisés à `{}`, les champs `[arr]` à `[]`
- Remplissez les valeurs ; les champs laissés vides sont omis du document final
- `Ctrl+S` → modal de confirmation → `insert_one` → rechargement automatique

> **Collection vide :** si la collection ne contient aucun document, `a` ouvre un écran sans champs. Insérez d'abord un document via l'éditeur MQL (`F5`), puis utilisez `a` pour les suivants.

### Suppression d'un document (`D`)

Depuis le Data Grid MongoDB (mode non read-only), appuyez sur `D` sur la ligne à supprimer :

- Un modal de confirmation affiche l'`_id` du document : `Delete document with _id: …?`
- `Y` → `delete_one` → rechargement automatique
- `N` / `Esc` → annulation sans effet
...
- **`[obj]`** : `Enter` → drill-in dans le sous-document (voir ci-dessous) ; `i` → édition du JSON brut directement dans le champ (ex. `{"city":"Paris","zip":"75001"}`)
...
- **`Ctrl+S`** depuis un niveau imbriqué affiche `"Esc: confirm & go back to parent"` — la sauvegarde vers MongoDB n'est possible que depuis le niveau racine
...
| MongoDB | Document | `mongodb` 3 | ✅ Supporté (`--features mongodb`) — browse, MQL, CRUD complet (insert `a`, edit `Enter`, delete `D`) + navigation nested |
