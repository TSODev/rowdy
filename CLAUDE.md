...
- [x] **Tri par colonne dans DataGrid** — touche `s` sur colonne courante → ORDER BY ASC/DESC/reset (cycle) ; indicateur `▲`/`▼` en vert dans l'en-tête ; réinjecté dans tous les spawn SQL (`spawn_reload_filters`, `spawn_load_more`, `spawn_load_all`, `EditSaved`) ; `sortable: bool` activé uniquement sur le DataGrid principal (SQL, non read-only) ; `A` charge toutes les lignes en une seule requête sans LIMIT (`spawn_load_all`)
- [ ] **Autocomplétion SQL (Tab)** — depuis l'éditeur SQL, `Tab` ouvre un popup de suggestions (noms de tables + colonnes de la table courante) basé sur `all_schemas` déjà chargé ; navigation `↑/↓` + `Enter` pour insérer ; `Esc` pour fermer
- [x] **Load All dans DataGrid** — `A` charge toutes les lignes restantes en une seule requête sans LIMIT (`spawn_load_all`) ; utilise `total_count` si connu, sinon 10 000 comme borne ; disponible uniquement sur DataGrid SQL principal quand `has_more = true`
